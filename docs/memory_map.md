...
|---|---|---|
| `+0` | `CFG` | `[3:0]` type (0 off, 1 LFO, 2 ADSR, 3 BUS — #44), `[5:4]` LFO shape (saw/pulse/tri/sine via osc_core), `[15:6]` target bus; LFO: `[31:16]` rate — low 16 bits of the UQ0.24 phase increment (5.7 mHz steps, 375 Hz max); ADSR: `[25:16]` gate bus (level-sensitive, > 0 = held); BUS: `[25:16]` SOURCE bus — stateless, reads the source bus's FIRMWARE base (walker contributions live only in the replicas), × DEPTH (`0x10000` = unity, sign inverts), chain-adds to target |
