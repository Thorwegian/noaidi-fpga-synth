# Noaidi â€” Consolidated Design Document
...
modulation model. Status marks: âœ… implemented & hardware-verified,
ðŸ”¨ in progress, ðŸ“‹ planned.

## Terminology (settled 2026-08-30)

- **Element** â€” what the FPGA *generates*: one oscillatorâ†’filterâ†’gain
  sound unit. The FPGA has 256 of them and no opinion about how they
  are used.
- **Lane** â€” how it generates them, technically: one of 256
  ...
  sound.
- **Voice** â€” a firmware-side *grouping* of elements (e.g. 8 detuned
...
- The user's scope of action via MIDI/control surfaces is **not yet
  nailed down** â€” firmware vocabulary above voice level stays open.
- Banned: "patch" for parameter data (everything is live; see
  Corrections). "Patch panel" survives only as the CV-routing metaphor.

## Vision

A hardware synthesizer with a **house sound** â€” a characteristic,
...
```
MIDI in â”€â”€â–º ESP32-C3 â”€â”€SPI masterâ”€â”€â–º Tang Nano 20K (GW2AR-18C)
            (MIDI parse, display,     â””â”€ all audio synthesis
             voice allocation)            â”€â”€â–º SPDIF + I2S out
```

- **ESP32-C3**: MIDI (UART1, 31250 baud), future display, and every
  *musical* decision â€” voice allocation, unison grouping, CC mapping.
...
- Sample rate **96 kHz**; SYSCLK **73.728 MHz = 768 Ã— 96 kHz**, from
...
- Per-board one-time setup: `pll_clk O0=73.728M -s` on the BL616
  ...
  dead pin 10 â€” see AGENTS.md bring-up notes.
- **Why not ~100 MHz** (decision 2026-08-30, Thor): five ear-verified
  silicon timing failures that STA passed â€” the fabric has no margin
  for the filter's 36Ã—36 DSP cascades at 98.304 MHz, and each fix
...
- **A gateware PLL is never a valid workaround.** A dead pin 10 means
  the clock chip wasn't programmed â€” fix the board setup, not the
...
- **The drum is the sole timebase**: one 768-slot counter yields the
  ...
  (every 6 slots; 768 = 128 cells Ã— 6). No other audio-rate counter
...
| Pitch / cutoff | UQ4.10 logâ‚‚ | 4-bit octave + 10-bit fraction; linearized via BSRAM LUTs (24-bit phase-delta LUT, 16-bit compressed SVF-K LUT), recycled per octave via barrel shifts |
| Phase accumulators | UQ0.24 | |
| Gains | UQ4.4 log | 6 dB per integer step, 0.375 dB per fraction step via 16-entry LUT + barrel shift; 0xFF â‰ˆ âˆ’96 dB |
| Envelope times | 8-bit logâ‚‚ | 4-bit octave + 4-bit 1/16-octave, decoded by the same LUT+shift machinery (ðŸ“‹) |

## The drum â€” SCMO pipeline âœ…

SCMO ("schmoe" â€” Single Clock Multiple Operation, i.e. pipelining),
...
pipeline on each of slots 0â€“255 (~25% of the drum budget; the idle
slots are reserved for LFOs, CV read-back and future work).

**Voice budget**: 32 voices of polyphony Ã— up to 8 unison voices per
keystroke = 256. The pipeline knows nothing of that grouping â€” 256
interchangeable voice slots; unison is a firmware convention.

**Per-element chain** (16 stages today): state/param RAM read â†’ LUT reads
â†’ oscillator (saw, pulse, triangle, sine; pitch, duty, phase reset) â†’
SVF 1 â†’ SVF 2 (shared type/cutoff/resonance; 12/24 dB via single/dual
mode â€” a separate filter per unison voice costs nothing in cycles) â†’
stereo log attenuation (independent L/R, the monoâ†’stereo point) â†’ mix
accumulate (26-bit, 8 guard bits, sat24 limiter) + state write-back.

**State banks**: semi dual-ported BSRAM, read at pipeline start,
written at pipeline end, 11 slots apart â€” no address collision.

**Parameter smoothing** ðŸ“‹ (deferred â€” unresolved dilemma, Thor
2026-08-30): the attractive design is a first-order LPF
`out <= out + ((in - out) >>> coeff)` applied **after** the LUT
lookups, per element, in the linear domain â€” interpolation between
log-domain LUT steps falls out for free; ~10 fractional guard bits so
`>>> 10` (Ï„ â‰ˆ 960 samples â‰ˆ 10 ms, matching MIDI's ~1k events/s
...
LUT, so a post-LUT smoother would low-pass the envelope as well â€”
...
base values, sum FPGA-generated modulators after the smoother â€” the
glide principle â€” but that forfeits free LUT interpolation for
modulated parameters). Not scheduled; revisit around ADSR design.

## Control plane â€” SPI + BSRAM ðŸ”¨

Authoritative detail: [memory_map.md](memory_map.md). Key stances:

- **Wire format â‰  storage geometry.** The SPI protocol speaks 16-bit
  addresses and 32-bit data words â€” a stable ABI with room to grow â€”
...
- CDC by construction: SPI writes on the sclk-clocked BSRAM port, the
  ...
  *semi* dual-port infers (`DPX9B`); **true** dual-port does not â€” so
...
- Parameter data is ping-pong double-buffered (half-active/half-shadow
  ...
  Atomicity is the swap's job â€” BSRAM does not give read-during-write
  coherency.
- Bring-up today âœ…: a 16-word byte-wide register file
...
- **CV table**: 256 anonymous 16-bit control voltages â€” the FPGA's only
  notion of external input. Firmware patches MIDI streams to cells once
  per program; each event is then one cell write.
- **Routes**: 8 cables per voice â€” source (CV / LFO / ADSR1 / ADSR2) â†’
...
- **I2S** (pins 54â€“56): self-clocked master, BCLK = sysclk/16.
...
**Playable synth.** MIDI keyboard â†’ 32 voices Ã— 8 elements (saw,
church-organ detune, cutoff tracking, velocity â†’ gain), clean at the
new 73.728 MHz SYSCLK **even with keys mashed** â€” no screaming, no
...
1. **Voice concept on the ESP32** (pulled ahead by Thor, in progress
   on `feat/voice-concept`) â€” engine link (sole SPI owner, 1 kHz tick)
   + voice allocator per
   [firmware_architecture.md](firmware_architecture.md). 32 voices Ã—
   8 elements, church-organ detune, cutoff one octave above the note,
   velocity â†’ gain, omni, steal-oldest. Gate-by-gain (no FPGA GATE bit
   yet): clicks expected. No stop/program structure yet.
2. **Housekeeping** âœ… (2026-08-30) â€” constants live in
   `synth_pkg.sv`, `*patch*` file names and voiceâ†’element/lane
   identifiers renamed. Verified zero-change: benches printed
   identical numbers before and after.
3. **GATE** â€” per-element gate bit, written through the swap like
   every other parameter. Hard on/off. Random per-element phase
   configured by the ESP32 would be nice here (Thor: "no rush").
4. **Afterwards, order TBD**: ADSR (amp first; needs GATE settled â€”
...
5. **Experiment, some point later (Thor)**: try removing the pipeline
   stage splits (S3B/S5B/S8B/S9B) now that SYSCLK is 73.728 MHz â€” they
