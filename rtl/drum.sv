...
// every 768 sysclk cycles (73.728 MHz / 96 kHz).  All time-multiplexed
...
//   voice_enter : high during the 256 slots in which an element enters
...
module drum #(
    parameter int CYCLES     = 768,
    parameter int NUM_VOICES = 256,
    parameter int CELL_DIV   = 6
) (
    input  logic                   clk,
    input  logic                   rst_n,
    ...
    logic [$clog2(CELL_DIV)-1:0] cell_cnt;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            cell_cnt <= '0;
        else if (slot_r == CYCLES - 1)          // realign at wrap (no-op
            cell_cnt <= '0;                     // while CYCLES % CELL_DIV == 0)
        else
            cell_cnt <= (cell_cnt == CELL_DIV - 1) ? '0 : cell_cnt + 1'b1;
    end
